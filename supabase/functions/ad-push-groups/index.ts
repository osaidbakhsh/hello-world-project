...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Transform and upsert groups in batches
    const batchSize = 100;
    let inserted = 0;
    let errors = 0;
    ...
    for (let i = 0; i < payload.items.length; i += batchSize) {
      ...
      const records = batch.map(group => ({
        ...
        group_scope: group.groupScope || null,
        group_type: group.groupType || null,
        is_privileged: group.isPrivileged ?? false,
      ...
      const { error: upsertError } = await supabaseAdmin
        .from('ad_groups')
        .upsert(records, { onConflict: 'domain_id,captured_at,dn' });

      if (upsertError) {
        console.error('Error upserting groups batch:', upsertError);
        errors += batch.length;
      } else {
    ...
    console.error('AD push groups error:', error);
