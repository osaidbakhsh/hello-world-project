...
`;
  ...
  const generateOpensslCommand = () => {
    ...
    const configFile = `csr_details.txt`;
...
# (Using configuration file - download csr_details.txt first)
...
# Set proper permissions on private key (Linux/Mac)
chmod 600 Private_SSL_Key.pem
...
`;
  ...
  const generateConfigForDownload = () => {
    const validSans = sans.filter(san => san.value.trim());
    const sanEntries = validSans.map((san, i) => 
      `${san.type}.${i + 2} = ${san.value}`
    ).join('\n');
...
commonName = ${cn}
emailAddress = admin@${cn.replace(/^www\./, '')}
        ...
        <TabsContent value="generate" className="space-y-4 mt-4">
          ...
          {opensslConfig && (
            <div className="space-y-4">
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex gap-2">
                      ...
                      <Button 
                        size="sm" 
                        variant="ghost"
                        onClick={() => downloadFile(opensslCommand, 'ssl_commands.txt')}
                      >
                        <Download className="h-4 w-4" />
                      </Button>
                    </div>
              ...
              {/* Download csr_details.txt button */}
              <div className="flex gap-2">
                <Button 
                  variant="outline"
                  className="flex-1"
                  onClick={() => downloadFile(generateConfigForDownload(), 'csr_details.txt')}
                >
                  <Download className="h-4 w-4 me-2" />
                  {t('itTools.downloadCsrConfig')}
                </Button>
                <Button 
                  variant="outline"
                  className="flex-1"
