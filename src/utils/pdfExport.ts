...
export const exportToPDF = (options: GenericPDFOptions) => {
  const { title, headers, rows, filename, isArabic = true } = options;
  ...
  const footer = isArabic 
    ? 'تم إنشاء التقرير بواسطة نظام إدارة البنية التحتية'
    : 'Generated by IT Infrastructure Management System';
  doc.text(footer, pageWidth / 2, finalY + 15, { align: 'center' });
